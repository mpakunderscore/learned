...
exports.getAllWords = async () => {
    return models.Word.findAll({limit: 3000, order: [['count', 'DESC']]});
};
...
exports.getTokenWords = async () => {
    ...
    let words = await models.Word.findAll({limit: 3000, order: [['count', 'DESC']]});
...
exports.getLinksSimpleList = async () => {
    ...
    let links = await models.Link.findAll({order: [['createdAt', 'DESC']]})
    let linksJson = [];
